...
def compute_score(sent, sents):
    """
    Computes the average score of sent vs the other sentences (the
    result of sent vs itself isn't counted because it's 1, and that's
    above UPPER_BOUND).
    """
    if not len(sent):
        return 0
...
def find_likely_body(b):
    ...
    """
    return max(b.find_all(), key=lambda t: len(
        t.find_all('p', recursive=False)))
...
class Summary(object):
    ...
    def __repr__(self):
        return 'Summary({0}, {1}, {2}, {3})'.format(
            repr(self.url), repr(self.article_html), repr(self.title),
            repr(self.summaries))
...
def summarize_page(url):
    ...
    summaries = map(
        lambda p: re.sub('\s+', ' ', summarize_block(p.text)).strip(),
        b.find_all('p'))
